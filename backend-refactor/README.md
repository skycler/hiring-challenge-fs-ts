...
## Documentation (20min)
- All code is now properly documented with docstrings and comments, which will help other developers understand the code and maintain it in the future.
- Add API documentation using OpenAPI/Swagger, which is automatically generated by FastAPI based on the code and data models. This will provide a clear and interactive interface for clients to understand how to use the API and what data to expect in the responses.
...
## Experimental: New API implementation from scratch (10min)
- New API implementation from scratch with SPecs-only approach (no code reference at all) - rust for performance and safety and axum for web framework.
- using same integration tests to validate the new implementation, which will ensure that the new API is functionally equivalent to the old one and meets the same requirements.